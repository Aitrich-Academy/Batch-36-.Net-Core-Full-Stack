...
namespace Domain.Service.Job
{
    public class JobRepository : IJobRepository
    {
        ...
        HireMeNowWebApContext _context;
        IMapper _mapper;
        static List<JobPost> joblist;

        public JobRepository(HireMeNowWebApContext context, IMapper mapper)
        {
            _context = context;
            _mapper = mapper;
        }
        public async Task<List<JobPost>> GetJobs()
        {
            ...
            try
            {
                var jobs = await _context.JobPosts.Include(e => e.Location).Include(e => e.Industry).Include(e => e.Company).Include(e => e.PostedByNavigation).Include(e => e.Category).ToListAsync();
                return jobs;
            }
        ...
        public async Task<PagedList<SavedJob>> GetAllSavedJobsOfSeeker(Guid jobseekerId, JobListParams param)
        {
            ...
            var query = _context.SavedJobs
              .OrderByDescending(c => c.DateSaved).Where(e => e.SavedBy == jobseekerId).Include(e => e.JobNavigation).AsQueryable();
